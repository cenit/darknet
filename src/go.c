...
int* calculate_liberties(float* board)
{
  ...
  for (j = 0; j < 19; ++j) {
    for (i = 0; i < 19; ++i) {
      ...
      int index = j * 19 + i;
      if (board[index] == 0) {
        if ((i > 0) && board[index - 1])
          propagate_liberty(board, lib, visited, j, i - 1, board[index - 1]);
        if ((i < 18) && board[index + 1])
          propagate_liberty(board, lib, visited, j, i + 1, board[index + 1]);
        if ((j > 0) && board[index - 19])
...
int generate_move(network net, int player, float* board, int multi, float thresh, float temp, char* ko, int print)
{
  ...
  if (thresh > move[indexes[0]])
    thresh = move[indexes[nind - 1]];

  for (i = 0; i < 19; ++i) {
    for (j = 0; j < 19; ++j) {
      if (move[i * 19 + j] < thresh)
        move[i * 19 + j] = 0;
    }
  }
...
void engine_go(char* filename, char* weightfile, int multi)
{
  ...
  while (1) {
    ...
    if (!strcmp(buff, "protocol_version")) {
      ...
      char comm[256];
      scanf("%s", comm);
      int known = (!strcmp(comm, "protocol_version") || !strcmp(comm, "name") || !strcmp(comm, "version") || !strcmp(comm, "known_command") || !strcmp(comm, "list_commands") || !strcmp(comm, "quit") || !strcmp(comm, "boardsize") || !strcmp(comm, "clear_board") || !strcmp(comm, "komi") || !strcmp(comm, "final_status_list") || !strcmp(comm, "play") || !strcmp(comm, "genmove"));
      if (known)
        printf("=%s true\n\n", ids);
      else
        printf("=%s false\n\n", ids);
      ...
      if (type[0] == 'd' || type[0] == 'D') {
        ...
        for (j = 0; j < 19; ++j) {
          for (i = 0; i < 19; ++i) {
            if (board[j * 19 + i] == 1)
              fprintf(f, "play black %c%d\n", 'A' + i + (i >= 8), 19 - j);
            if (board[j * 19 + i] == -1)
              fprintf(f, "play white %c%d\n", 'A' + i + (i >= 8), 19 - j);
            if (board[j * 19 + i])
              ++count;
          }
        }
    ...
    if (strlen(line) == 0 || dnum) {
      ...
      if (c <= 'T' && c >= 'A') {
        ...
        if (col > 7 && noi)
          col -= 1;
        if (num == 3)
          board[row * 19 + col] = (g == 'b') ? color : -color;
      } else if (c == 'c') {
        char g;
        int num = sscanf(line, "%c %c %d", &g, &c, &row);
        row = (inverted) ? 19 - row : row - 1;
...
void run_go(int argc, char** argv)
{
  ...
  if (0 == strcmp(argv[2], "train"))
    train_go(cfg, weights);
  else if (0 == strcmp(argv[2], "valid"))
    valid_go(cfg, weights, multi);
